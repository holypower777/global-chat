...
import { isArrayEqual, splitIntoColumns } from 'platform-components/src/utils';
...
const Rubricator = React.memo(({ channels = [], handleSelect, selectedItem }: RubricatorProps) => {
    ...
    return isArrayEqual(prevProps.channels.map((e) => e.map((j) => j.userId)).flat(), nextProps.channels.map((e) => e.map((j) => j.userId)).flat())
        && prevProps.selectedItem?.userId === nextProps.selectedItem?.userId;
});
