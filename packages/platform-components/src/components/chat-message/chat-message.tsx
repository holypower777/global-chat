        ...
        return (
            <div className={cx(b('chat-message', { highlited }), mix)}>
                <div className={b('chat-message', 'container')}>
                    {showMessageTime ? (
                        <Text mix={b('chat-message', 'time')} title={getFullDate(time, true, true)}>
                            {getHoursAndMinutes(time)}
                        </Text>
                    ) : null}
                    {channelName ? (
                        <Text mix={b('chat-message', 'username')} weight={Text.WEIGHT.L}>
                            {channelName}:
                        </Text>
                    ) : null}
                    {showBadges ? (
                        <ChatMessageBadges badges={badges} subBadges={subBadges} />
                    ) : null}
                    <Text mix={b('chat-message', 'username')} style={style} weight={Text.WEIGHT.L}>
                        {username}:
                    </Text>
